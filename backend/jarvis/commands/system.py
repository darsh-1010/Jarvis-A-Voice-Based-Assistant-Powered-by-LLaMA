...
def open_app(app_name: str) -> bool:
    ...
    if app_name.lower() in apps:
        ...
        log_action("SYS_APP", f"Opening {app_name} | Path: {path}", f"I'm launching {app_name} for you.")
...
def open_source_file() -> None:
    ...
    log_action("SYS_FILE", f"Explorer: {config.source_file_path}", "I'm opening your project folder.")
    try:
        subprocess.Popen(f'explorer.exe "{config.source_file_path}"')
    except Exception as exc:
        log_action("SYS_FILE_ERR", f"Error: {exc}", "I had trouble opening the folder.", level=logging.ERROR)
...
async def take_screenshot() -> str:
    ...
    log_action("SYS_SCREENSHOT", "PyAutoGUI trigger (3s delay)", "I'm taking a screenshot of your screen in 3 seconds.")
    await asyncio.sleep(3)
    screenshot = await asyncio.to_thread(pyautogui.screenshot)
    file_path = "screenshot.png"
    await asyncio.to_thread(screenshot.save, file_path)
    log_action("SYS_SCREENSHOT_DONE", f"Path: {file_path}", "I've saved the screenshot to your project folder.")
    return file_path
...
def shutdown_system() -> None:
    ...
    log_action("SYS_POWER", "Shell: shutdown /s /t 60", "I'm shutting down the computer in 60 seconds.", level=logging.WARNING)
